...
func main() {
	var (
		httpAddr   = flag.String("addr", defaultAddr, "World service address.")
	...
	log.Printf("[INFO] World service with HTTP check listening on %s", *httpAddr)
...
func newServer(cfgFile string) *server {
	...
	s.router.HandleFunc("GET", "/world", s.handleWorld())
...
func (s *server) handleWorld() http.HandlerFunc {
